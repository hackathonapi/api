...
def generate_clearview(
    ...
    # Safety / Objectivity / Bias — only shown when OpenAI analysis is absent
    # (when OpenAI responded, the same content appears on the AI Analysis page)
    if not ai_sections:
    ...
    # ── AI Analysis page (last page, only when OpenAI responded) ───────────
    if ai_sections:
        pdf.add_page()

        pdf.set_font("Helvetica", "B", 16)
        pdf.set_text_color(15, 15, 15)
        pdf.cell(_TEXT_WIDTH, 10, txt="AI Analysis", align="L")
        pdf.ln(4)

        pdf.set_font("Helvetica", "I", 8.5)
        pdf.set_text_color(150, 150, 150)
        pdf.cell(_TEXT_WIDTH, 5, txt="Generated by OpenAI gpt-4o-mini", align="L")
        pdf.ln(10)

        _divider(pdf)

        for section_name, section_text in ai_sections.items():
            _section_label(pdf, section_name)
            _section_body(pdf, section_text)
            _divider(pdf)
